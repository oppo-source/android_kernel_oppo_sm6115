...
static struct subsystem_data ss_name[] = {
	{ "modem", 605, 1 },
	{ "adsp", 606, 2 },
	{ "cdsp", 607, 5 },
//	{ "apss", 631, -1 },
...
#if IS_ENABLED(CONFIG_QCOM_SMEM)
//	n_subsystems = of_property_count_strings(node, "ss_name");
	n_subsystems = ARRAY_SIZE(ss_name);
	if (n_subsystems < 0)
		goto exit;

	for (i = 0; i < n_subsystems; i++) {
//		of_property_read_string_index(node, "ss_name", i, &name);
		for (k = 0; k < n_subsystems; k++)
		for (j = 0; j < ARRAY_SIZE(subsystems); j++) {
			if (!strcmp(subsystems[j].name, ss_name[i].name)) {
...
//	n_subsystems = of_property_count_strings(node, "ss-name");
	n_subsystems = ARRAY_SIZE(ss_name);
	if (n_subsystems < 0) {
		pr_err("ss_name_op_subsystem\n");
		return length;
	}

	for (i = 0; i < n_subsystems; i++) {
//		of_property_read_string_index(node, "ss-name", i, &name);
		for (l = 0; l < n_subsystems; l++)
